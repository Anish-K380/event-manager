...
create table wallet (
    ...
    constraint fk_no_duplicate foreign key (id) references users(id)
);
...
create table transactions (
    ...
    constraint c_promo_consistency check (
        (promo is null and promo_scope is null) or
...
create table ledger (
    ...
    constraint change_positive check (
        (change < 0) or (alter_type in ('topup', 'refund'))
    ),
    constraint change_negative check (
        (change > 0) or (alter_type = 'purchase')
    )
);
